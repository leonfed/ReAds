...
for filename in masks_files:
    ...
    for i in range(len(masks)):
        ...
        for c in contours:
            # c2 = np.array([[[2708, 1279],
            #                 [2703, 730],
            #                 [627, 748],
            #                 [622, 1287]]])
            # hull = cv2.convexHull(c, returnPoints=True)
            cv2.drawContours(tmp_image, c, -1, (randint(0, 255), randint(0, 255), randint(0, 255)), 3)
            # peri = cv2.arcLength(hull, True)
            # approx = cv2.approxPolyDP(hull, 0.02 * peri, True)
            # if len(approx) == 4:
            # cv2.drawContours(original_image, [approx], -1, (0, 255, 0), 3)
            # cv2.drawContours(tmp_image, [approx], -1, (randint(0, 255), randint(0, 255), randint(0, 255)), 3)
