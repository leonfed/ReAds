...
shutil.rmtree('../video/data/masks')
os.makedirs('../video/data/masks')
...
for filename in masks_files:
    print(filename)
    masks = np.load('../video/data/mask_rcnn_output/%s.npy' % filename)
    scores = np.load('../video/data/mask_rcnn_output/scores_%s.npy' % filename)
    ...
    if best_mask_score < 0.85:
        ...
        np.save('../video/data/masks/%s' % filename, np.array(masks[best_mask_index]))
